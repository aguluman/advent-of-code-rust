...
pub fn part1(input: &str) -> u64 {
    ...
    loop {
        ...
        let next_pos = match dir {
            Direction::Up => if i >= 1 { Some((i - 1, j)) } else { None },
            Direction::Right => if j < n - 1 { Some((i, j + 1)) } else { None },
            Direction::Down => if i < n - 1 { Some((i + 1, j)) } else { None },
            Direction::Left => if j >= 1 { Some((i, j - 1)) } else { None },
        };
        ...
        if next_pos.is_none() || (next_pos.is_some() && map[next_pos.unwrap().0][next_pos.unwrap().1] == '#') {
...
pub fn part2(input: &str) -> u64 {
    ...
    fn find_loop(map: &[Vec<char>], start_i: usize, start_j: usize, start_dir: Direction) -> bool {
        ...
        while !queue.is_empty() {
            ...
            let next_pos = match dir {
                Direction::Up => if i >= 1 { Some((i - 1, j)) } else { None },
                Direction::Right => if j + 1 < n { Some((i, j + 1)) } else { None },
                Direction::Down => if i + 1 < n { Some((i + 1, j)) } else { None },
                Direction::Left => if j >= 1 { Some((i, j - 1)) } else { None },
            };
